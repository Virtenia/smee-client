...
class SmeeClient {
    ...
    #onmessage = async (msg) => {
        ...
        if (data.bodyEncoding !== "base64" || typeof data.body !== "string") {
            this.#logger.error(`[smee] Incompatible server: expected base64-encoded body but received bodyEncoding=${JSON.stringify(data.bodyEncoding)}, body type=${typeof data.body}. Upgrade the smee server.`);
            return;
        }
